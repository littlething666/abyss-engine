...
- [ ] **PR-2E (Hardening).** Cross-pipeline E2E suite, SSE resume tests for new pipelines, cancel race tests per pipeline, OpenRouter request-shape lockstep extended to all four call sites, durable-repo retry tests covering `{ stage }` + `{ jobId }`. Pending.
- [ ] **Per-kind routing in `generationClient.ts`**. The `NEXT_PUBLIC_DURABLE_RUNS_KINDS` flag and per-kind `selectRepoFor` function described in `plans/phase2.md` are not yet implemented in the frontend. The `generationClient.ts` currently uses a binary `durableRuns` flag. Pending.
- [ ] **Frontend Supersedes-Key header passthrough**. The `apiClient.ts` and `DurableGenerationRunRepository.ts` need to forward `Supersedes-Key` for topic-expansion submissions. Pending.
