...
Last updated: 2026-05-05. Reflects Phase 0 complete and Phase 0.5 in progress (Generation Client seam: repository contract + local adapter + **GenerationClient facade** + **entry-path routing through `submitRun` / prepare helpers** + **bootstrap `ensureGenerationClientRegistered`** + **feature-owned `ArtifactApplier`s** (dedupe-backed expansion supersession + lattice-hash Stage B gate) + **`applied_artifacts` Dexie dedupe store v2** landed in workspace). PRs are stacked: each step's PR targets the previous step's branch as its base.
