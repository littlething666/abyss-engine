...
14. **Cloudflare runtime test harness and first D1/route proofs have landed.** `backend/vitest.runtime.config.ts` runs `src/runtimeTests/**/*.runtime.test.ts` through `@cloudflare/vitest-pool-workers` against the Worker entrypoint and wrangler bindings. Runtime tests now reset/apply `backend/d1/reset.sql` + `backend/d1/init.sql`, prove real-D1 `atomicSubmitRun` same-key concurrency/idempotency/budget rollback behavior, prove semantic-keyed event idempotency under concurrent `appendTypedOnce` calls, and cover Worker route boundaries for invalid submit payloads, dispatch failure with no orphan queued run, and retry child-run lineage.
...
- Cloudflare runtime test harness (`backend/vitest.runtime.config.ts`, `pnpm --filter abyss-durable-orchestrator test:runtime`) with real local D1 schema reset/init helpers and runtime tests for `atomicSubmitRun` concurrency/idempotency/budget rollback, semantic-keyed event idempotency, Worker submit validation, dispatch failure/no-orphan-queued behavior, and retry child-run lineage.
...
### Remaining recommended follow-ups in order of priority (remove items as they are completed)
 ...
 1. Continue real Cloudflare runtime tests beyond the landed D1/event/route proofs: R2 artifact write/read behavior, Workflow retry/replay faults, cancellation/cache-hit/supersession boundaries.
...
3. **Add real Cloudflare runtime integration tests for durability-critical paths**
   ...
   - Worker route runtime coverage now proves malformed submit payloads fail before run writes, workflow dispatch failure marks the run failed instead of leaving it queued, and retry creates a child run with `parent_run_id` lineage.
   - **Remaining:** Workflow step retry/replay behavior, R2 artifact writes/reads, cancellation before/after stage boundaries, and cache-hit/supersession materialization behavior.
