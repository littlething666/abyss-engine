...
### Remaining recommended follow-ups in order of priority (remove items as they are completed)
 ...
 1. Continue real Cloudflare runtime tests beyond the landed D1/R2/event/route proofs: Workflow retry/replay faults, cancellation before/mid-stage boundaries, and cache-hit/supersession materialization behavior.
...
3. **Add real Cloudflare runtime integration tests for durability-critical paths**
   ...
   - Worker route runtime coverage now proves malformed submit payloads fail before run writes, workflow dispatch failure marks the run failed instead of leaving it queued, terminal ready-run cancellation is non-destructive, and retry creates a child run with `parent_run_id` lineage.
   - **Remaining:** Workflow step retry/replay behavior, cancellation before/mid-stage boundaries, and cache-hit/supersession materialization behavior.
