...
14. **Cloudflare runtime test harness and first D1/R2/route proofs have landed.** `backend/vitest.runtime.config.ts` runs `src/runtimeTests/**/*.runtime.test.ts` through `@cloudflare/vitest-pool-workers` against the Worker entrypoint and wrangler bindings. Runtime tests now reset/apply `backend/d1/reset.sql` + `backend/d1/init.sql`, prove real-D1 `atomicSubmitRun` same-key concurrency/idempotency/budget rollback behavior, prove semantic-keyed event idempotency under concurrent `appendTypedOnce` calls, prove R2 artifact write/read plus D1 artifact cache-key uniqueness, and cover Worker route boundaries for invalid submit payloads, dispatch failure with no orphan queued run, cache-hit Learning Content materialization before terminal completion, ready-run cancellation immutability, and retry child-run lineage.
...
- Cloudflare runtime test harness (`backend/vitest.runtime.config.ts`, `pnpm --filter abyss-durable-orchestrator test:runtime`) with real local D1 schema reset/init helpers and runtime tests for `atomicSubmitRun` concurrency/idempotency/budget rollback, semantic-keyed event idempotency, R2 artifact write/read behavior, D1 artifact cache-key uniqueness, Worker submit validation, dispatch failure/no-orphan-queued behavior, ready-run cancellation immutability, and retry child-run lineage.
...
### Remaining recommended follow-ups in order of priority (remove items as they are completed)
 ...
 1. Continue real Cloudflare runtime tests beyond the landed D1/R2/event/route proofs: Workflow retry/replay faults, cancellation before/mid-stage boundaries, and supersession materialization behavior.
...
3. **Add real Cloudflare runtime integration tests for durability-critical paths**
   ...
   - R2 runtime coverage now proves artifact JSON writes/reads through the real local bucket binding and D1 artifact metadata remains unique for concurrent writes to the same `(device_id, kind, input_hash)` cache key.
   - Worker route runtime coverage now proves malformed submit payloads fail before run writes, workflow dispatch failure marks the run failed instead of leaving it queued, cache-hit submit materializes Learning Content from R2 before `run.completed` without dispatching a Workflow, terminal ready-run cancellation is non-destructive, and retry creates a child run with `parent_run_id` lineage.
   - **Remaining:** Workflow step retry/replay behavior, cancellation before/mid-stage boundaries, and supersession materialization behavior.
