...
const E2E_CRYSTAL_TRIAL_QUESTIONS: CrystalTrialScenarioQuestion[] = [
  {
    id: 'q1',
...
const abyssDev: AbyssDev = {
  ...
  triggerTrial: async (topicId: string) => {
    ...
    trialStore.markTrialQuestionsReady(ref);
  ...
  submitTrialCorrect: async (topicId: string) => {
    ...
    const ref = { subjectId, topicId };
    return useCrystalTrialStore.getState().forceCompleteWithCorrectAnswers(ref, E2E_CRYSTAL_TRIAL_QUESTIONS);
  },
  ...
  submitTrialWrong: async (topicId: string) => {
    ...
    for (const q of E2E_CRYSTAL_TRIAL_QUESTIONS) {
      trialStore.answerQuestion(ref, q.id, q.correctAnswer === 'correct' ? 'wrong' : 'correct');
    }
    const result = trialStore.submitTrial(ref, E2E_CRYSTAL_TRIAL_QUESTIONS);
    if (result) {
