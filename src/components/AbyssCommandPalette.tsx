...
function loadStudyCardFilterFromStorage(): StudyCardFilterState {
  ...
  try {
    ...
    if (
      ...
      (parsed as { v?: number }).v === 2 && 'base' in parsed && 'mini' in parsed
    ) {
    ...
    if (flat && typeof flat.FLASHCARD === 'boolean' && typeof flat.MINI_GAME === 'boolean') {
      ...
      return mergeStudyCardFilter({
        base: { FLASHCARD: flat.FLASHCARD, SINGLE_CHOICE: flat.SINGLE_CHOICE ?? true, MULTI_CHOICE: flat.MULTI_CHOICE ?? true },
        mini: { CATEGORY_SORT: allMini, SEQUENCE_BUILD: allMini, CONNECTION_WEB: allMini },
      });
    }
    return fallback;
...
export function AbyssCommandPalette({
  open, onOpenChange, isDebugMode, onSummarizeScreen, onOpenSubjectCurriculum, onStartStudyWithCardTypes,
  ...
  const canStartFilteredStudy = Boolean(selectedTopicId) && (enabledBaseTypes.length > 0 || enabledMiniGameTypes.length > 0) && Boolean(onStartStudyWithCardTypes);
  ...
  const handleDevAddXp = () => {
    const ref = uiStore.getState().selectedTopicRef;
    if (!ref) return;
    const progression = useProgressionStore.getState();
    const nextXp = progression.addXP(ref, DEV_XP_AMOUNT, { sessionId: 'dev-command-palette' });
    if (nextXp <= 0) return;
    appEventBus.emit('xp:gained', { subjectId: ref.subjectId, topicId: ref.topicId, amount: DEV_XP_AMOUNT, sessionId: 'dev-command-palette', cardId: 'dev-command-palette' });
    onOpenChange(false);
  };
  ...
  const handleDevXpBuffToggle = () => { useProgressionStore.getState().toggleBuffFromCatalog(DEV_XP_BUFF_ID, DEV_BUFF_SOURCE); onOpenChange(false); };
      ...
      <Command className="rounded-lg! border-none! bg-transparent! shadow-none!">
        ...
        <CommandList>
          ...
          <CommandGroup heading="Study">
            <CommandItem value="timeline study history" onSelect={handleOpenTimeline}><History className="size-4" /><span>Open study timeline</span></CommandItem>
            <CommandItem value="discovery wisdom altar" onSelect={handleOpenDiscovery}><Landmark className="size-4" /><span>Open Wisdom Altar (Discovery)</span></CommandItem>
          </CommandGroup>
          {onStartStudyWithCardTypes ? (
            <>
              <CommandSeparator />
              <CommandGroup heading="Card type filter">
                <CommandItem value="study filtered cards selected topic crystal flashcard choice mini game" disabled={!canStartFilteredStudy} onSelect={() => { if (!canStartFilteredStudy) return; onStartStudyWithCardTypes(studySelection); onOpenChange(false); }}><BookOpen className="size-4" /><span>Study filtered cards (selected topic)</span></CommandItem>
                {BASE_CARD_TYPES_ORDER.map((type) => {
                  const on = studyCardFilter.base[type];
                  const label = BASE_CARD_TYPE_LABELS[type];
                  return (<CommandItem key={type} value={`filter include ${label} ${type} study filter toggle`} onSelect={() => setStudyCardFilter((prev) => ({ ...prev, base: { ...prev.base, [type]: !prev.base[type] } }))}>{on ? <Check className="size-4 text-primary" aria-hidden /> : <Circle className="size-4 text-muted-foreground" aria-hidden />}<span>Include {label}</span></CommandItem>);
                })}
              </CommandGroup>
              <CommandGroup heading="Mini games">
                {MINI_GAME_TYPES_ORDER.map((type) => {
                  ...
                  return (<CommandItem key={type} value={`filter include mini game ${label} ${type} study filter toggle`} onSelect={() => setStudyCardFilter((prev) => ({ ...prev, mini: { ...prev.mini, [type]: !prev.mini[type] } }))}>{on ? <Check className="size-4 text-primary" aria-hidden /> : <Circle className="size-4 text-muted-foreground" aria-hidden />}<span>Include {label}</span></CommandItem>);
                })}
              </CommandGroup>
            </>
          ) : null}
          {onSummarizeScreen ? (<CommandGroup heading="Assistant"><CommandItem value="screen capture summarize screenshot assistant vision" onSelect={() => { onSummarizeScreen(); onOpenChange(false); }}><Camera className="size-4" /><span>Summarize screen with assistant</span></CommandItem></CommandGroup>) : null}
          {onOpenSubjectCurriculum ? (<CommandGroup heading="Curriculum"><CommandItem value="new subject curriculum graph generate indexeddb" onSelect={() => { onOpenSubjectCurriculum(); onOpenChange(false); }}><Network className="size-4" /><span>New subject from prompt (curriculum graph)</span></CommandItem></CommandGroup>) : null}
          {isDebugMode ? (
            <CommandGroup heading="Dev">
              <CommandItem value="add xp crystal dev" disabled={!canDevAddXp} onSelect={() => { if (!canDevAddXp) return; handleDevAddXp(); }}><Sparkles className="size-4" /><span>Add +80 XP to selected crystal</span></CommandItem>
              <CommandItem value="subtract xp crystal dev" disabled={!canDevAddXp} onSelect={() => { if (!canDevAddXp) return; handleDevSubtractXp(); }}><Minus className="size-4" /><span>Subtract 80 XP from selected crystal</span></CommandItem>
              <CommandItem value="toggle xp multiplier buff dev" onSelect={handleDevXpBuffToggle}><Zap className="size-4" /><span>{devXpBuffActive ? 'Turn off 5\u00D7 XP multiplier (dev)' : 'Turn on 5\u00D7 XP multiplier (dev)'}</span></CommandItem>
            </CommandGroup>
          ) : null}
        </CommandList>
      </Command>
