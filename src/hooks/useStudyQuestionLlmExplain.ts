...
function cacheKey(cardId: string, topicLabel: string, q: string, agentPersonality: string): string {
  return `${cardId}\0${topicLabel}\0${q}\0${agentPersonality}`;
}
...
export function useStudyQuestionLlmExplain({
  ...
  const requestExplain = useCallback(() => {
    ...
    const key = cacheKey(cardId, topicLabel, questionText, agentPersonality);
        ...
        for await (const chunk of client.stream(
          {
            ...
            intent: { topicLabel, questionText, agentPersonality },
          },
          ac.signal,
        )) {
  ...
  }, [cardId, topicLabel, questionText, agentPersonality, setPending]);
