 ...
 * ## Layer
 *
 * This module lives in `backend/src/contracts/` (not `routes/`) so it can
 * be imported by typed event builders, workflow code, and the route layer
 * without violating backend layering.
 *
 * ## Lockstep policy
 *
 ...
 *
 * Unknown statuses **throw** rather than returning as-is.
 * This is a backend boundary — invalid DB status values indicate schema
 * drift and must fail loudly per AGENTS.md.
 */
export function dbStatusToTransport(status: string): TransportRunStatus {
  ...
  if (mapped === undefined) {
    throw new Error(
      `dbStatusToTransport: unknown DB status "${status}". ` +
      `Valid statuses: ${Object.keys(DB_TO_TRANSPORT_STATUS).join(', ')}`,
    );
  }
  return mapped;
}
...
export function transportStatusToDb(status: string): DbRunStatus {
  ...
  throw new Error(
    `transportStatusToDb: unknown transport status "${status}". ` +
    `Valid statuses: ${Object.values(DB_TO_TRANSPORT_STATUS).join(', ')}`,
  );
}
