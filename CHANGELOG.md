...
- Added backend-resolved current Crystal Trial question-set reads: the Worker derives the active card-pool hash from Learning Content Store cards, the frontend reads through an infrastructure repository/query hook, and persisted trial UI state no longer stores generated questions.
- Narrowed Crystal Trial browser state to player attempt/cooldown data only: generated questions and card-pool hashes are no longer stored in Zustand/localStorage, submission/evaluation receives the backend current-set questions transiently, and stale shared frontend artifact-applier contracts were removed.
- Added a local-workflow removal boundary guard that fails if local runners, the generation HUD, frontend generation logs/stores, frontend artifact appliers, or browser-owned generation-policy seams are reintroduced.
